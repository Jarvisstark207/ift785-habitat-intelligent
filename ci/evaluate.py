...
class CodeEvaluator:
    ...
    def check_conventional_commits(self):
        ...
        import re
        conventional_count = sum(1 for msg in messages if re.match(conventional_pattern, msg))
            ...
            if check_type == 'import_test':
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        self.details.append(f"✓ {check['name']} ({check_points}/{check['points']})")
                    else:
                        self.details.append(f"✗ {check['name']} (0/{check['points']})")
                except:
                    self.details.append(f"✗ {check['name']} - erreur (0/{check['points']})")
                ...
                if num_classes >= min_classes:
                    check_points = check['points']
                    self.details.append(f"✓ {check['name']}: {num_classes} classes ({check_points}/{check['points']})")
                else:
                    partial = int((num_classes / min_classes) * check['points'])
                    check_points = partial
                    self.details.append(f"⚠ {check['name']}: {num_classes}/{min_classes} classes ({partial}/{check['points']})")
                ...
                if os.path.exists('app.py'):
                    with open('app.py', 'r') as f:
                        ...
                        if 'endpoint' in check:
                            ...
                            if f'@app.get("{endpoint}")' in content or f"@app.get('{endpoint}')" in content or \
                               f'@app.post("{endpoint}")' in content or f"@app.post('{endpoint}')" in content:
                                check_points = check['points']
                                self.details.append(f"✓ {check['name']} ({check_points}/{check['points']})")
                            else:
                                self.details.append(f"✗ {check['name']} (0/{check['points']})")
                            ...
                            found = []
                            for ep in endpoints:
                                if f'@app.get("{ep}")' in content or f"@app.get('{ep}')" in content or \
                                   f'@app.post("{ep}")' in content or f"@app.post('{ep}')" in content:
                                    found.append(ep)
                            
                            if len(found) == len(endpoints):
                                check_points = check['points']
                                self.details.append(f"✓ {check['name']}: tous presents ({check_points}/{check['points']})")
                            elif len(found) > 0:
                                partial = int((len(found) / len(endpoints)) * check['points'])
                                check_points = partial
                                self.details.append(f"⚠ {check['name']}: {len(found)}/{len(endpoints)} endpoints ({partial}/{check['points']})")
                            else:
                                self.details.append(f"✗ {check['name']}: aucun endpoint (0/{check['points']})")
    ...
    def evaluate_tests(self):
        ...
        if os.path.exists('coverage.xml'):
            try:
                ...
                if coverage >= min_cov:
                    points += cov_weight
                    self.details.append(f"✓ Couverture {coverage:.1f}% >= {min_cov}% ({cov_weight:.1f}/{cov_weight:.1f})")
                elif coverage >= min_cov - 20:
                    partial = cov_weight * 0.6
                    points += partial
                    self.details.append(f"⚠ Couverture {coverage:.1f}% ({partial:.1f}/{cov_weight:.1f})")
                else:
                    partial = cov_weight * 0.3
                    points += partial
                    self.details.append(f"✗ Couverture {coverage:.1f}% < {min_cov-20}% ({partial:.1f}/{cov_weight:.1f})")
            except:
    ...
    def _evaluate_tests_simple(self):
        ...
        if os.path.exists('tests'):
            try:
                ...
                if result.returncode == 0:
                    points += weight * 0.7
                    self.details.append(f"✓ Tests passent ({weight*0.7:.1f}/{weight*0.7:.1f})")
                else:
                    points += weight * 0.35
                    self.details.append(f"⚠ Tests echouent partiellement ({weight*0.35:.1f}/{weight*0.7:.1f})")
            except:
    ...
    def _evaluate_test_structure(self):
        ...
        for category, specs in structure.items():
            ...
            if files_present == len(required_files):
                ...
                self.details.append(f"✓ Tests {category}: fichiers presents ({cat_weight*0.5:.1f})")
            elif files_present > 0:
                partial = (files_present / len(required_files)) * cat_weight * 0.5
                cat_points += partial
                self.details.append(f"⚠ Tests {category}: {files_present}/{len(required_files)} fichiers ({partial:.1f})")
            else:
                self.details.append(f"✗ Tests {category}: fichiers manquants (0/{cat_weight*0.5:.1f})")
            ...
            if min_tests > 0:
                ...
                if test_count >= min_tests:
                    cat_points += cat_weight * 0.5
                    self.details.append(f"✓ Tests {category}: {test_count} tests ({cat_weight*0.5:.1f})")
                elif test_count > 0:
                    partial = (test_count / min_tests) * cat_weight * 0.5
                    cat_points += partial
                    self.details.append(f"⚠ Tests {category}: {test_count}/{min_tests} tests ({partial:.1f})")
                else:
                    self.details.append(f"✗ Tests {category}: aucun test (0/{cat_weight*0.5:.1f})")
        ...
        if os.path.exists('flake8_report.json'):
            try:
                with open('flake8_report.json', 'r') as f:
                    ...
                    if total_errors == 0:
                        points += weight * 0.5
                        self.details.append(f"✓ Flake8: 0 erreur ({weight*0.5:.1f}/{weight*0.5:.1f})")
                    elif total_errors <= max_errors:
                        partial = weight * 0.3
                        points += partial
                        self.details.append(f"⚠ Flake8: {total_errors} erreurs ({partial:.1f}/{weight*0.5:.1f})")
                    else:
                        points += weight * 0.1
                        self.details.append(f"✗ Flake8: {total_errors} erreurs ({weight*0.1:.1f}/{weight*0.5:.1f})")
            except:
                points += weight * 0.25
                self.details.append(f"⚠ Erreur flake8 ({weight*0.25:.1f}/{weight*0.5:.1f})")
            ...
            try:
                with open('complexity_report.json', 'r') as f:
                    ...
                    if high_complexity == 0:
                        points += weight * 0.5
                        self.details.append(f"✓ Complexite acceptable ({weight*0.5:.1f}/{weight*0.5:.1f})")
                    elif high_complexity <= 3:
                        partial = weight * 0.3
                        points += partial
                        self.details.append(f"⚠ {high_complexity} fonctions complexes ({partial:.1f}/{weight*0.5:.1f})")
                    else:
                        points += weight * 0.1
                        self.details.append(f"✗ {high_complexity} fonctions complexes ({weight*0.1:.1f}/{weight*0.5:.1f})")
            except:
                points += weight * 0.25
                self.details.append(f"⚠ Erreur complexite ({weight*0.25:.1f}/{weight*0.5:.1f})")
    ...
    def evaluate_git(self):
        ...
        if all_meet:
            points += weight * 0.4
            self.details.append(f"✓ Commits minimums atteints ({weight*0.4:.1f}/{weight*0.4:.1f})")
        else:
            below = [n for n, c in self.contributors.items() if c['commits'] < min_commits]
            points += weight * 0.2
            self.details.append(f"⚠ Commits insuffisants pour: {', '.join(below)} ({weight*0.2:.1f}/{weight*0.4:.1f})")
        
        # Equilibre contribution
        balanced, ratios = self.analyze_contribution_balance()
        if balanced:
            points += weight * 0.3
            self.details.append(f"✓ Contribution equilibree ({weight*0.3:.1f}/{weight*0.3:.1f})")
        else:
            points += weight * 0.1
            ratio_str = ', '.join(f"{n}: {r}%" for n, r in ratios.items())
            self.details.append(f"⚠ Desequilibre: {ratio_str} ({weight*0.1:.1f}/{weight*0.3:.1f})")
        ...
        if conv_ratio >= min_conv:
            points += weight * 0.3
            self.details.append(f"✓ Commits conventionnels: {conv_ratio:.0f}% ({weight*0.3:.1f}/{weight*0.3:.1f})")
        else:
            partial = (conv_ratio / min_conv) * weight * 0.3
            points += partial
            self.details.append(f"⚠ Commits conventionnels: {conv_ratio:.0f}% ({partial:.1f}/{weight*0.3:.1f})")
    ...
    def run_evaluation(self):
        ...
        print(f"{'Fonctionnalite':20s}: {scores['functionality']:.1f}/{self.config['functionality']['weight']}")
        print(f"{'Tests':20s}: {scores['tests']:.1f}/{self.config['tests']['weight']}")
        print(f"{'Qualite Code':20s}: {scores['quality']:.1f}/{self.config['quality']['weight']}")
        print(f"{'Git/Commits':20s}: {scores['git']:.1f}/{self.config['git']['weight']}")
...
"""
        ...
        balance_text = 'Equilibree' if data['contribution_balanced'] else 'Desequilibree'

...
class TestHistoryFilters:
    ...
    def test_filter_by_sensor_type_only(self, mock_repo):
        """Test filtrage par type de capteur uniquement"""
        mock_repo.find_with_filters.return_value = []

        service = HistoryService(mock_repo)
        result = service.get_filtered_history(sensor_type="temperature")
    ...
    def test_filter_by_start_date_only(self, mock_repo):
        """Test filtrage par date de début uniquement"""
        mock_repo.find_with_filters.return_value = []

        service = HistoryService(mock_repo)
        result = service.get_filtered_history(start_date="2026-01-01T00:00:00")
    ...
    def test_filter_by_end_date_only(self, mock_repo):
        """Test filtrage par date de fin uniquement"""
        mock_repo.find_with_filters.return_value = []

        service = HistoryService(mock_repo)
        result = service.get_filtered_history(end_date="2026-01-31T23:59:59")
    ...
    def test_filter_with_limit(self, mock_repo):
        """Test filtrage avec limite de résultats"""
        mock_repo.find_with_filters.return_value = []

        service = HistoryService(mock_repo)
        result = service.get_filtered_history(limit=50)

        call_args = mock_repo.find_with_filters.call_args[1]
        assert call_args["limit"] == 50

    def test_filter_combined_location_and_type(self, mock_repo):
        """Test filtrage combiné location + type"""
        mock_repo.find_with_filters.return_value = []

        service = HistoryService(mock_repo)
        result = service.get_filtered_history(location="salon", sensor_type="temperature")
    ...
    def test_filter_combined_all_params(self, mock_repo):
        """Test filtrage avec tous les paramètres"""
        mock_repo.find_with_filters.return_value = []
    ...
    def test_filter_returns_correct_count(self, mock_repo):
        """Test que le count correspond au nombre de résultats"""
    ...
    def test_filter_includes_applied_filters_in_response(self, mock_repo):
        """Test que les filtres appliqués sont dans la réponse"""
        mock_repo.find_with_filters.return_value = []
...
class TestHourlyStatsFilters:
    ...
    def test_hourly_filter_by_location(self, mock_repo):
        """Test filtrage horaire par location"""
        mock_repo.find_with_filters.return_value = []

        service = StatsAdvancedService(mock_repo)
        result = service.calculate_hourly_stats(location="salon", date="2026-02-03")

        call_args = mock_repo.find_with_filters.call_args[1]
        assert call_args["location"] == "salon"

    def test_hourly_filter_by_sensor_type(self, mock_repo):
        """Test filtrage horaire par type de capteur"""
        mock_repo.find_with_filters.return_value = []

        service = StatsAdvancedService(mock_repo)
        result = service.calculate_hourly_stats(sensor_type="consommation", date="2026-02-03")

        call_args = mock_repo.find_with_filters.call_args[1]
        assert call_args["sensor_type"] == "consommation"

    def test_hourly_filter_by_date(self, mock_repo):
        """Test filtrage horaire par date spécifique"""
        mock_repo.find_with_filters.return_value = []
    ...
    def test_filter_response_metadata(self, mock_repo):
        """Test métadonnées de filtrage dans la réponse"""
        mock_repo.find_with_filters.return_value = []
